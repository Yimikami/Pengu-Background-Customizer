...
let uiVisible=!1,debounceTimeout,skinData=[],universeData=[],skinLineData=[],tftData=[],previewGroups=[],backgroundEnabled=!0,currentOpacity=.3,storedOpacity=currentOpacity,persistBackground=!1,centeredSplash=!0,settingsVisible=!1,cycleShuffleEnabled=!1,cycleInterval=30,currentSearchQuery="",shuffleCycleIntervalId=null,transitionDuration=.5,lastAppliedUrl=null,skinProfiles=[],activeProfile=null,isInitialLoad=!0;function isDataStoreAvailable(){return void 0!==window.DataStore}function saveSettings(){try{let e={backgroundEnabled,currentOpacity:storedOpacity,persistBackground,centeredSplash,cycleShuffleEnabled,cycleInterval,transitionDuration,skinProfiles,activeProfile,savedAt:new Date().toISOString()};isDataStoreAvailable()?(DataStore.set("dynamicBg_config",e),console.log("Settings saved:",e)):console.error("DataStore API not available")}catch(t){console.error("Failed to save settings:",t)}}function loadSavedSettings(){try{if(isDataStoreAvailable()){let e=DataStore.get("dynamicBg_config");if(e)return backgroundEnabled=void 0===e.backgroundEnabled||e.backgroundEnabled,currentOpacity=storedOpacity=void 0!==e.currentOpacity?parseFloat(e.currentOpacity):.3,persistBackground=void 0!==e.persistBackground&&e.persistBackground,centeredSplash=void 0===e.centeredSplash||e.centeredSplash,cycleShuffleEnabled=void 0!==e.cycleShuffleEnabled&&e.cycleShuffleEnabled,cycleInterval=void 0!==e.cycleInterval?parseInt(e.cycleInterval):30,transitionDuration=void 0!==e.transitionDuration?parseFloat(e.transitionDuration):.5,skinProfiles=void 0!==e.skinProfiles?e.skinProfiles:[],activeProfile=void 0!==e.activeProfile?e.activeProfile:null,console.log("Loaded settings:",{backgroundEnabled,currentOpacity:storedOpacity,persistBackground,centeredSplash,cycleShuffleEnabled,cycleInterval,transitionDuration,skinProfiles,activeProfile,savedAt:e.savedAt}),!0}return console.log("No saved settings, using defaults"),!1}catch(t){return console.error("Failed to load settings:",t),!1}}function preloadImage(e){return new Promise(t=>{if(!e)return t();let o=new Image;o.src=e,o.onload=t,o.onerror=()=>{console.warn(`Failed to preload image: ${e}`),t()}})}async function applyBackground(e){let t=document.getElementById("rcp-fe-viewport-root");if(!t||!e||!backgroundEnabled){removeBackground();return}console.log(`Applying background for ${e.name} with opacity: ${currentOpacity}`);let o=centeredSplash?e.splashPath||e.backgroundTextureLCU||e.uncenteredSplashPath:e.uncenteredSplashPath||e.splashPath||e.backgroundTextureLCU;if(o===lastAppliedUrl){let a=document.getElementById("client-bg-container");if(a){let n=a.querySelector(".client-bg-layer:last-child");n&&parseFloat(n.style.opacity)!==currentOpacity&&(n.style.opacity=currentOpacity,console.log(`Updated opacity to ${currentOpacity} for unchanged background: ${e.name}`))}return}await preloadImage(o);let r=document.getElementById("client-bg-container");r||((r=document.createElement("div")).id="client-bg-container",r.style.cssText=`
            ...
            height: 100%;
            z-index: -1;
        `,t.appendChild(r),t.classList.add("custom-background"));let l=r.querySelectorAll(".client-bg-layer");l.length>1&&l.forEach((e,t)=>{t<l.length-1&&(e.remove(),console.log(`Removed excess layer: ${e.style.backgroundImage}`))});let i=document.createElement("div");i.className="client-bg-layer",i.style.cssText=`
        position: absolute;
        top: 0;
        left: 0;
        width: 100%;
    ...
    `,r.appendChild(i);let s=document.getElementById("client-bg-style");s||((s=document.createElement("style")).id="client-bg-style",document.head.appendChild(s)),s.textContent=`
        .custom-background {
            position: relative;
        }
        .client-bg-layer {
    ...
    `,i.offsetHeight,i.style.opacity=currentOpacity;let d=r.querySelector(".client-bg-layer:not(:last-child)");d&&(d.style.opacity=0,setTimeout(()=>{d.parentNode&&(d.remove(),console.log(`Cleaned up old layer: ${d.style.backgroundImage}`))},1e3*transitionDuration+100)),lastAppliedUrl=o,console.log(`Background applied: ${e.name}, URL: ${o}, Opacity: ${currentOpacity}, Transition: ${transitionDuration}s`)}function removeBackground(){let e=document.getElementById("rcp-fe-viewport-root");if(e&&e.classList.contains("custom-background")){let t=document.getElementById("client-bg-container");if(t){let o=t.querySelectorAll(".client-bg-layer");o.forEach(e=>{e.style.opacity=0,setTimeout(()=>{e.parentNode&&(e.remove(),console.log(`Removed layer during reset: ${e.style.backgroundImage}`))},1e3*transitionDuration+100)}),setTimeout(()=>{t.parentNode&&(t.remove(),console.log("Removed background container"))},1e3*transitionDuration+100)}e.classList.remove("custom-background"),lastAppliedUrl=null,console.log("Background fully removed")}}function checkAndApplyBackground(){let e=document.getElementById("rcp-fe-viewport-root");if(!e)return;let t=document.querySelector('[data-screen-name="rcp-fe-lol-parties"]'),o=document.querySelector('.screen-root.active[data-screen-name="rcp-fe-lol-activity-center"]'),a=document.querySelector('.screen-root[data-screen-name="rcp-fe-lol-postgame"]'),n=document.querySelector('.screen-root[data-screen-name="rcp-fe-lol-profiles-main"]'),r=DataStore.get("selectedSkin");currentOpacity=o&&"1"===getComputedStyle(o).opacity||a&&"1"===getComputedStyle(a).opacity||n&&"1"===getComputedStyle(n).opacity?0:storedOpacity,backgroundEnabled&&r&&(t||persistBackground)?applyBackground(r):removeBackground()}function setupActivityCenterObserver(){let e=document.querySelector('.screen-root.active[data-screen-name="rcp-fe-lol-activity-center"]');if(!e){console.log("Activity center not found for observer setup");return}let t=new MutationObserver(()=>{checkAndApplyBackground()});t.observe(e,{attributes:!0,attributeFilter:["style","class"]}),console.log("Activity center observer set up")}function setupProfilesMainObserver(){let e=document.querySelector('.screen-root[data-screen-name="rcp-fe-lol-profiles-main"]');if(!e){console.log("Profiles main screen not found for observer setup");return}let t=new MutationObserver(()=>{checkAndApplyBackground()});t.observe(e,{attributes:!0,attributeFilter:["style","class"]}),console.log("Profiles main screen observer set up")}function setupPostgameObserver(){let e=document.querySelector('.screen-root[data-screen-name="rcp-fe-lol-postgame"]');if(!e){console.log("Postgame screen not found for observer setup");return}let t=new MutationObserver(()=>{checkAndApplyBackground()});t.observe(e,{attributes:!0,attributeFilter:["style","class"]}),console.log("Postgame screen observer set up")}function checkAndCreateButton(){clearTimeout(debounceTimeout),debounceTimeout=setTimeout(()=>{let e=document.querySelector('[data-screen-name="rcp-fe-lol-parties"]'),t=document.getElementById("client-bg-show-button");if(!e){t&&t.remove();return}t||createShowButton(e)},100)}function createShowButton(e){let t=document.getElementById("client-bg-hover-area");t&&t.remove();let o=document.createElement("div");o.id="client-bg-hover-area",o.style.cssText=`
        position: absolute;
        top: 40%;
        left: -70px;
        width: 250px;
    ...
    `,a.appendChild(n);let r=document.createElement("div");r.style.cssText=`
        flex: 1;
        display: flex;
        flex-direction: column;
        gap: 15px;
    ...
    `,r.className="profiles-content";let l=document.createElement("div");l.style.cssText="display: flex; align-items: center; gap: 10px; margin-bottom: 15px;";let i=document.createElement("input");i.type="text",i.placeholder=`Profile ${skinProfiles.length+1}`,i.style.cssText=`
        flex: 1;
        background: #010a13;
        border: 1px solid #785a28;
        color: #cdbe91;
    ...
    `,s.addEventListener("mouseover",()=>{s.style.background="#1e2328",s.style.borderColor="#c8aa6e",s.style.color="#f0e6d2"}),s.addEventListener("mouseout",()=>{s.style.background="#1e2328",s.style.borderColor="#785a28",s.style.color="#cdbe91"});let d=document.createElement("span");d.style.cssText="color: #ff5555; font-size: 12px; display: none; margin-top: 5px;",l.appendChild(i),l.appendChild(s),r.appendChild(l),r.appendChild(d);let c=document.createElement("div");function p(){if(c.innerHTML="",0===skinProfiles.length){let e=document.createElement("div");e.textContent="No profiles saved",e.style.cssText=`
            ...
            `,n.addEventListener("click",()=>{console.log(`Loading profile: ${e.name}`),DataStore.set("favoriteSkins",e.skins),window.favoriteSkins=e.skins||[],window.isFavoritesToggled=DataStore.get("favoritesToggled")||!1,activeProfile=e.name,saveSettings(),p();let t=document.getElementById("client-bg-customizer-ui");if(t){let o=t.querySelector(".main-window");if(o&&window.renderSkins){console.log("Preparing to render skins with:",{favoriteSkinsCount:window.favoriteSkins.length,isFavoritesToggled:window.isFavoritesToggled,previewGroupsCount:previewGroups.length,searchQuery:currentSearchQuery});let a=t.querySelector(".filter-dropdown .dropdown-toggle"),n=t.querySelector(".filter-dropdown .dropdown-menu");if(a&&n){let r=window.isFavoritesToggled?"Favorites":"All Skins";a.textContent=r,n.querySelectorAll(".dropdown-item").forEach(e=>{e.classList.toggle("selected",e.textContent===r)}),console.log(`Filter dropdown updated to: ${r}`)}else console.warn("Filter dropdown or menu not found");let l=window.isFavoritesToggled?"favorites":"all";console.log(`Rendering skins with filter: ${l}`),window.renderSkins(previewGroups,currentSearchQuery,l)}else console.error("Main window or renderSkins missing")}else console.error("Customizer UI not found")});let r=document.createElement("button");r.textContent="Save",r.style.cssText=`
                ...
                border-radius: 2px;
                cursor: pointer;
            `,r.addEventListener("click",()=>{let t=DataStore.get("favoriteSkins")||[];if(0===t.length){d.textContent="No favorites to save",d.style.display="block",setTimeout(()=>{d.style.display="none"},3e3);return}e.skins=t,saveSettings(),p()});let l=document.createElement("button");l.textContent="\uD83D\uDDD1",l.style.cssText=`
                padding: 4px 8px;
                background: #1e2328;
                border: 1px solid #785a28;
                color: #cdbe91;
                border-radius: 2px;
                cursor: pointer;
            `,l.addEventListener("click",()=>{skinProfiles=skinProfiles.filter(t=>t!==e),activeProfile===e.name&&(activeProfile=null),saveSettings(),p()});let i=document.createElement("button");i.textContent="↑",i.style.cssText=`
                padding: 4px 8px;
                background: #1e2328;
                border: 1px solid #785a28;
                color: #cdbe91;
    ...
    `,a.appendChild(n);let r=document.createElement("div");r.style.cssText=`
        ...
        scrollbar-width: thin;
        scrollbar-color: #785a28 transparent;
    `,r.className="settings-content";let l=document.createElement("style");l.textContent=`
    ...
    `,document.head.appendChild(l);let i=document.createElement("h4");i.textContent="General",i.style.cssText=`
        color: #f0e6d2;
        font-size: 18px;
        font-weight: bold;
        margin: 10px 0 5px 0;
    ...
    `,R.addEventListener("click",()=>{let e=Math.max(parseInt(M.min),10*Math.round((parseInt(M.value)-10)/10));M.value=e,cycleInterval=e,V.textContent=e.toString(),saveSettings()});let H=document.createElement("button");H.textContent=">",H.style.cssText=`
        ...
        border-radius: 2px;
        transition: color 0.2s, border-color 0.2s;
    `,H.addEventListener("click",()=>{let e=Math.min(parseInt(M.max),10*Math.round((parseInt(M.value)+10)/10));M.value=e,cycleInterval=e,V.textContent=e.toString(),saveSettings()}),R.addEventListener("mouseover",()=>{R.style.color="#f0e6d2",R.style.borderColor="#c8aa6e"}),R.addEventListener("mouseout",()=>{R.style.color="#cdbe91",R.style.borderColor="#785a28"}),H.addEventListener("mouseover",()=>{H.style.color="#f0e6d2",H.style.borderColor="#c8aa6e"}),H.addEventListener("mouseout",()=>{H.style.color="#cdbe91",H.style.borderColor="#785a28"}),j.appendChild(R),j.appendChild(M),j.appendChild(H),j.appendChild(V),r.appendChild(j),M.addEventListener("input",()=>{cycleInterval=parseInt(M.value),V.textContent=cycleInterval.toString(),saveSettings()});let Q=document.createElement("label");Q.textContent="Transition Duration (Seconds):",Q.style.cssText="margin-bottom: 5px;",r.appendChild(Q);let K=document.createElement("div");K.style.cssText=`
        display: flex;
        align-items: center;
        gap: 15px;
        margin-bottom: 15px;
    ...
    `,X.addEventListener("click",()=>{let e=Math.max(parseFloat(W.min),Math.round((parseFloat(W.value)-.1)*10)/10);W.value=e,transitionDuration=e,Y.textContent=e.toString(),saveSettings()});let Z=document.createElement("button");Z.textContent=">",Z.style.cssText=`
        ...
        border-radius: 2px;
        transition: color 0.2s, border-color 0.2s;
    `,Z.addEventListener("click",()=>{let e=Math.min(parseFloat(W.max),Math.round((parseFloat(W.value)+.1)*10)/10);W.value=e,transitionDuration=e,Y.textContent=e.toString(),saveSettings()}),X.addEventListener("mouseover",()=>{X.style.color="#f0e6d2",X.style.borderColor="#c8aa6e"}),X.addEventListener("mouseout",()=>{X.style.color="#cdbe91",X.style.borderColor="#785a28"}),Z.addEventListener("mouseover",()=>{Z.style.color="#f0e6d2",Z.style.borderColor="#c8aa6e"}),Z.addEventListener("mouseout",()=>{Z.style.color="#cdbe91",Z.style.borderColor="#785a28"}),K.appendChild(X),K.appendChild(W),K.appendChild(Z),K.appendChild(Y),r.appendChild(K),W.addEventListener("input",()=>{transitionDuration=parseFloat(W.value),Y.textContent=transitionDuration.toString(),saveSettings()}),a.appendChild(r),o.appendChild(a);let J=document.createElement("button");J.textContent="Close",J.className="settings-close-button",J.style.cssText=`
        margin-top: 20px;
        margin-left: auto;
        margin-right: auto;
        width: 50%;
