...
let uiVisible=!1,debounceTimeout,skinData=[],universeData=[],skinLineData=[],tftData=[],previewGroups=[],backgroundEnabled=!0,currentOpacity=.3,storedOpacity=currentOpacity,persistBackground=!1,centeredSplash=!0,settingsVisible=!1,cycleShuffleEnabled=!1,cycleInterval=30,currentSearchQuery="",shuffleCycleIntervalId=null,transitionDuration=.5,lastAppliedUrl=null,skinProfiles=[],activeProfile=null,isInitialLoad=!0;function isDataStoreAvailable(){return void 0!==window.DataStore}function saveSettings(){try{let e={backgroundEnabled,currentOpacity:storedOpacity,persistBackground,centeredSplash,cycleShuffleEnabled,cycleInterval,transitionDuration,skinProfiles,activeProfile,savedAt:new Date().toISOString()};isDataStoreAvailable()?(DataStore.set("dynamicBg_config",e),console.log("Settings saved:",e)):console.error("DataStore API not available")}catch(t){console.error("Failed to save settings:",t)}}function loadSavedSettings(){try{if(isDataStoreAvailable()){let e=DataStore.get("dynamicBg_config");if(e)return backgroundEnabled=void 0===e.backgroundEnabled||e.backgroundEnabled,currentOpacity=storedOpacity=void 0!==e.currentOpacity?parseFloat(e.currentOpacity):.3,persistBackground=void 0!==e.persistBackground&&e.persistBackground,centeredSplash=void 0===e.centeredSplash||e.centeredSplash,cycleShuffleEnabled=void 0!==e.cycleShuffleEnabled&&e.cycleShuffleEnabled,cycleInterval=void 0!==e.cycleInterval?parseInt(e.cycleInterval):30,transitionDuration=void 0!==e.transitionDuration?parseFloat(e.transitionDuration):.5,skinProfiles=void 0!==e.skinProfiles?e.skinProfiles:[],activeProfile=void 0!==e.activeProfile?e.activeProfile:null,console.log("Loaded settings:",{backgroundEnabled,currentOpacity:storedOpacity,persistBackground,centeredSplash,cycleShuffleEnabled,cycleInterval,transitionDuration,skinProfiles,activeProfile,savedAt:e.savedAt}),!0}return console.log("No saved settings, using defaults"),!1}catch(t){return console.error("Failed to load settings:",t),!1}}function preloadImage(e){return new Promise(t=>{if(!e)return t();let o=new Image;o.src=e,o.onload=t,o.onerror=()=>{console.warn(`Failed to preload image: ${e}`),t()}})}function preloadVideo(e){return new Promise(t=>{if(!e)return t();let o=document.createElement("video");o.src=e,o.preload="auto",o.onloadeddata=t,o.onerror=()=>{console.warn(`Failed to preload video: ${e}`),t()}})}async function applyBackground(e){let t=document.getElementById("rcp-fe-viewport-root");if(!t||!e||!backgroundEnabled){removeBackground();return}console.log(`Applying background for ${e.name} with opacity: ${currentOpacity}`);let o=centeredSplash?e.splashPath||e.backgroundTextureLCU||e.uncenteredSplashPath:e.uncenteredSplashPath||e.splashPath||e.backgroundTextureLCU;if(o===lastAppliedUrl){let a=document.getElementById("client-bg-container");if(a){let l=a.querySelector(".client-bg-layer:last-child");l&&parseFloat(l.style.opacity)!==currentOpacity&&(l.style.opacity=currentOpacity,console.log(`Updated opacity to ${currentOpacity} for unchanged background: ${e.name}`))}return}let n=o.toLowerCase().endsWith(".webm");n?await preloadVideo(o):await preloadImage(o);let r=document.getElementById("client-bg-container");r||((r=document.createElement("div")).id="client-bg-container",r.style.cssText=`
            ...
            height: 100%;
            z-index: -1;
        `,t.appendChild(r),t.classList.add("custom-background"));let i=r.querySelectorAll(".client-bg-layer");i.length>1&&i.forEach((e,t)=>{t<i.length-1&&(e.remove(),console.log(`Removed excess layer: ${"VIDEO"===e.tagName?e.src:e.style.backgroundImage}`))});let s;n?((s=document.createElement("video")).className="client-bg-layer",s.src=o,s.loop=!0,s.muted=!0,s.autoplay=!0,s.playsInline=!0,s.style.cssText=`
            position: absolute;
            top: 0;
            left: 0;
            width: 100%;
        ...
        `,s.onloadeddata=()=>{console.log(`Video loaded successfully: ${o}`),s.style.opacity=currentOpacity},s.onerror=()=>{console.error(`Failed to load video: ${o}`),s.remove(),removeBackground()}):((s=document.createElement("div")).className="client-bg-layer",s.style.cssText=`
            position: absolute;
            top: 0;
            left: 0;
            width: 100%;
    ...
    `,f.addEventListener("click",()=>{let e=Math.max(parseFloat(h.min),Math.round((parseFloat(h.value)-.1)*10)/10);h.value=e,storedOpacity=e,b.textContent=e.toString(),currentOpacity=storedOpacity,saveSettings(),checkAndApplyBackground()});let $=document.createElement("button");$.textContent=">",$.style.cssText=`
        ...
        border-radius: 2px;
        transition: color 0.2s, border-color 0.2s;
    `,$.addEventListener("click",()=>{let e=Math.min(parseFloat(h.max),Math.round((parseFloat(h.value)+.1)*10)/10);h.value=e,storedOpacity=e,b.textContent=e.toString(),currentOpacity=storedOpacity,saveSettings(),checkAndApplyBackground()}),h.addEventListener("input",()=>{storedOpacity=parseFloat(h.value),b.textContent=storedOpacity.toString(),currentOpacity=storedOpacity,saveSettings(),checkAndApplyBackground()}),f.addEventListener("mouseover",()=>{f.style.color="#f0e6d2",f.style.borderColor="#c8aa6e"}),f.addEventListener("mouseout",()=>{f.style.color="#cdbe91",f.style.borderColor="#785a28"}),$.addEventListener("mouseover",()=>{$.style.color="#f0e6d2",$.style.borderColor="#c8aa6e"}),$.addEventListener("mouseout",()=>{$.style.color="#cdbe91",$.style.borderColor="#785a28"}),m.appendChild(f),m.appendChild(h),m.appendChild($),m.appendChild(b),n.appendChild(m);let x=document.createElement("div");x.className="toggle-btn",x.style.cssText="margin-bottom: 15px;";let v=document.createElement("span");v.textContent="Keep background on all screens:";let y=document.createElement("label");y.className="toggle-switch";let k=document.createElement("input");k.type="checkbox",k.checked=persistBackground;let _=document.createElement("span");_.className="toggle-slider",y.appendChild(k),y.appendChild(_),x.appendChild(v),x.appendChild(y),n.appendChild(x),k.addEventListener("change",()=>{persistBackground=k.checked,saveSettings(),checkAndApplyBackground()});let w=document.createElement("div");w.className="toggle-btn",w.style.cssText="margin-bottom: 15px;";let C=document.createElement("span");C.textContent="Centered Splash:";let L=document.createElement("label");L.className="toggle-switch";let E=document.createElement("input");E.type="checkbox",E.checked=centeredSplash;let S=document.createElement("span");S.className="toggle-slider",L.appendChild(E),L.appendChild(S),w.appendChild(C),w.appendChild(L),n.appendChild(w),E.addEventListener("change",()=>{centeredSplash=E.checked,saveSettings(),checkAndApplyBackground()});let P=document.createElement("div");P.className="toggle-btn",P.style.cssText="margin-bottom: 15px;";let T=document.createElement("span");T.textContent="Enable TFT Content:";let I=document.createElement("label");I.className="toggle-switch";let B=document.createElement("input");B.type="checkbox",B.checked=!1!==DataStore.get("tftEnabled");let A=document.createElement("span");A.className="toggle-slider",I.appendChild(B),I.appendChild(A),P.appendChild(T),P.appendChild(I),n.appendChild(P),B.addEventListener("change",()=>{let e=B.checked;if(DataStore.set("tftEnabled",e),!e){let t=DataStore.get("selectedSkin");t&&t.isTFT&&(DataStore.set("selectedSkin",null),removeBackground(),console.log("Cleared selected TFT skin"))}saveSettings();let o=document.getElementById("client-bg-customizer-ui");if(o){generatePreviewGroups("champion");let a=o.querySelector(".main-window");a&&window.renderSkins&&window.renderSkins(previewGroups)}});let z=document.createElement("h4");z.textContent="Shuffle Settings",z.style.cssText=`
        color: #f0e6d2;
        font-size: 18px;
        font-weight: bold;
        margin: 10px 0 5px 0;
        text-transform: uppercase;
    `,n.appendChild(z);let F=document.createElement("hr");F.style.cssText=`
        border: 0;
        border-top: 1px solid #785a28;
        margin: 10px 0;
    `,n.appendChild(F);let N=document.createElement("div");N.className="toggle-btn",N.style.cssText="margin-bottom: 15px;";let D=document.createElement("span");D.textContent="Cycle Shuffle:";let O=document.createElement("label");O.className="toggle-switch";let q=document.createElement("input");q.type="checkbox",q.checked=cycleShuffleEnabled;let V=document.createElement("span");V.className="toggle-slider",O.appendChild(q),O.appendChild(V),N.appendChild(D),N.appendChild(O),n.appendChild(N),q.addEventListener("change",()=>{cycleShuffleEnabled=q.checked,saveSettings()});let G=document.createElement("label");G.textContent="Cycle Shuffle Interval (Seconds):",G.style.cssText="margin-bottom: 5px;",n.appendChild(G);let U=document.createElement("div");U.style.cssText=`
        display: flex;
        align-items: center;
        gap: 15px;
        margin-bottom: 15px;
    ...
    `;let M=document.createElement("span");M.textContent=cycleInterval.toString(),M.style.width="40px";let R=document.createElement("button");R.textContent="<",R.style.cssText=`
        ...
        border-radius: 2px;
        transition: color 0.2s, border-color 0.2s;
    `,R.addEventListener("click",()=>{let e=Math.max(parseInt(j.min),10*Math.round((parseInt(j.value)-10)/10));j.value=e,cycleInterval=e,M.textContent=e.toString(),saveSettings()});let H=document.createElement("button");H.textContent=">",H.style.cssText=`
        color: #cdbe91;
        font-size: 14px;
        font-family: 'LoL Display', 'BeaufortforLOL', sans-serif;
        font-weight: bold;
        ...
        border-radius: 2px;
        transition: color 0.2s, border-color 0.2s;
    `,H.addEventListener("click",()=>{let e=Math.min(parseInt(j.max),10*Math.round((parseInt(j.value)+10)/10));j.value=e,cycleInterval=e,M.textContent=e.toString(),saveSettings()}),R.addEventListener("mouseover",()=>{R.style.color="#f0e6d2",R.style.borderColor="#c8aa6e"}),R.addEventListener("mouseout",()=>{R.style.color="#cdbe91",R.style.borderColor="#785a28"}),H.addEventListener("mouseover",()=>{H.style.color="#f0e6d2",H.style.borderColor="#c8aa6e"}),H.addEventListener("mouseout",()=>{H.style.color="#cdbe91",H.style.borderColor="#785a28"}),U.appendChild(R),U.appendChild(j),U.appendChild(H),U.appendChild(M),n.appendChild(U),j.addEventListener("input",()=>{cycleInterval=parseInt(j.value),M.textContent=cycleInterval.toString(),saveSettings()});let Q=document.createElement("label");Q.textContent="Transition Duration (Seconds):",Q.style.cssText="margin-bottom: 5px;",n.appendChild(Q);let K=document.createElement("div");K.style.cssText=`
        display: flex;
        align-items: center;
        gap: 15px;
        margin-bottom: 15px;
    ...
    `,X.addEventListener("click",()=>{let e=Math.max(parseFloat(W.min),Math.round((parseFloat(W.value)-.1)*10)/10);W.value=e,transitionDuration=e,Y.textContent=e.toString(),saveSettings()});let Z=document.createElement("button");Z.textContent=">",Z.style.cssText=`
        ...
        border-radius: 2px;
        transition: color 0.2s, border-color 0.2s;
    `,Z.addEventListener("click",()=>{let e=Math.min(parseFloat(W.max),Math.round((parseFloat(W.value)+.1)*10)/10);W.value=e,transitionDuration=e,Y.textContent=e.toString(),saveSettings()}),X.addEventListener("mouseover",()=>{X.style.color="#f0e6d2",X.style.borderColor="#c8aa6e"}),X.addEventListener("mouseout",()=>{X.style.color="#cdbe91",X.style.borderColor="#785a28"}),Z.addEventListener("mouseover",()=>{Z.style.color="#f0e6d2",Z.style.borderColor="#c8aa6e"}),Z.addEventListener("mouseout",()=>{Z.style.color="#cdbe91",Z.style.borderColor="#785a28"}),K.appendChild(X),K.appendChild(W),K.appendChild(Z),K.appendChild(Y),n.appendChild(K),W.addEventListener("input",()=>{transitionDuration=parseFloat(W.value),Y.textContent=transitionDuration.toString(),saveSettings()}),a.appendChild(n),o.appendChild(a);let J=document.createElement("button");J.textContent="Close",J.className="settings-close-button",J.style.cssText=`
        margin-top: 20px;
        margin-left: auto;
        margin-right: auto;
        width: 50%;
